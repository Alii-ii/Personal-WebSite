...
#### Requirement 9：简历页移动端信息收敛到 footer 菜单

WHEN 访客在移动端访问 `/resume`，THE SYSTEM SHALL 隐藏正文侧栏中的节点目录与联系方式按钮，并通过与作品集页一致的 footer 菜单入口承载这些操作。

#### Scenario 9.1：正文信息减负
- **GIVEN** 浏览器视口宽度 < 768px
- **WHEN** 简历页渲染
- **THEN** 姓名、英文名和职业简介继续显示
- **AND** 节点目录与下载 PDF、微信、邮箱、Figma、小红书按钮不在正文中显示

#### Scenario 9.2：footer 菜单迁移
- **GIVEN** 访客点击简历页移动端 footer 的菜单按钮
- **WHEN** 全屏菜单展开
- **THEN** 菜单沿用作品集页现有 AppMenu 的布局、排版和底栏样式
- **AND** 中间菜单项替换为简历节点目录，点击后关闭菜单并滚动到对应节点
- **AND** AppMenu 底栏的联系方式按钮替换为简历页原有下载 PDF、微信、邮箱、Figma、小红书行为

#### Scenario 9.3：桌面端不回归
- **GIVEN** 浏览器视口宽度 ≥ 768px
- **WHEN** 简历页渲染
- **THEN** 左侧节点目录与联系方式按钮保持现状
- **AND** 不显示移动端 footer 菜单按钮

### 约束与边界

- 复用 `AppMenu` 的视觉壳，不复制一套新菜单样式。
- 目录锚点继续使用 `RESUME_SECTIONS` 中的稳定 `id`。
- 只迁移移动端的信息入口与操作位置，不修改简历正文内容、桌面布局和下载/复制/外链行为。

### 置信度评估

| 需求 | 置信度 | 说明 |
|------|--------|------|
| R9 简历移动端菜单 | 90% | 隐藏范围、复用样式及需要保留的点击事件均可从现有代码明确映射 |
...
#### Requirement 2：L3 与 Figma 原型的快捷键作用域隔离

WHEN L3 当前活动 frame 是 Figma 原型，THE SYSTEM SHALL 仅在访客实际进入该原型进行交互时将键盘事件交由 Figma；未进入原型或已切换到其他 frame 时，快捷键继续服务于 L3 导航。

#### Scenario 2.4：Figma 原型未激活时由 L3 响应
- **GIVEN** 当前活动 frame 是 Figma 原型，但访客尚未点击或聚焦原型内容
- **WHEN** 访客按下 L3 的左右、上下、Tab 切换、目录、评论或返回快捷键
- **THEN** L3 按既有规则处理快捷键
- **AND** Figma 原型不应提前截获这些按键

#### Scenario 2.5：进入 Figma 原型后由原型响应
- **GIVEN** 当前活动 frame 是 Figma 原型
- **WHEN** 访客点击并进入原型内容，使焦点切入 Figma iframe
- **THEN** Figma 自身的键盘交互生效
- **AND** L3 不应同时执行切页或其他全局快捷键操作

#### Scenario 2.6：离开 Figma 原型后恢复 L3 快捷键
- **GIVEN** 访客此前已进入 Figma 原型
- **WHEN** 访客通过 L3 的进度轴、相邻页点击、拖拽、tab 切换、项目切换或其他外层导航离开该 frame
- **THEN** 后续键盘事件重新由 L3 处理
- **AND** 非 Figma frame 的快捷键行为与本次修改前保持一致

#### Scenario 2.7：进入项目后立即预热 Figma 原型
- **GIVEN** 当前 L3 项目包含 Figma prototype frame
- **WHEN** 访客进入项目详情，且尚未切到原型所在 tab
- **THEN** 系统立即触发该项目全部 Figma URL 的预加载
- **AND** 切换到原型所在 tab 时不应卸载或重新触发预加载器
- **AND** 正式展示的 Figma iframe 保持独立布局，避免复用隐藏 iframe 导致画布缩放或空白

#### Scenario 2.8：L3 首屏以 frame 骨架承接加载
- **GIVEN** 访客首次进入 L3，舞台尺寸尚未完成测量或 frame 内容尚未加载
- **WHEN** 浏览器执行首轮绘制
- **THEN** 页面在目标 frame 的比例与边界内展示加载骨架
- **AND** 原图、iframe 或其他素材不得脱离 frame 容器铺满页面
- **AND** 图片或 iframe 加载完成后，内容在原位置淡入，不引起 frame 几何跳变

**← 变更说明**：原 Requirement 2.3 的“键盘可用于多维度导航”继续保留；本轮补充键盘所有权规则，将 Figma 预加载节点前移到进入项目时，并用同步比例与 frame 骨架承接 L3 首屏加载。

### 约束与边界

- 父页面只能可靠判断 L3 当前活动 frame 以及用户是否主动进入 iframe，不能读取跨域 Figma 原型内部当前节点或页面。
- Figma iframe 只有在当前 frame 处于活动态时才允许接收指针与键盘焦点；非活动 Figma frame 不应截获交互。
- 不尝试把 Figma 内部 `keydown` 冒泡到父页面，也不依赖当前不存在的 Figma `postMessage` 协议。
- 保持非 Figma 原型、图片、富文本、代码原型、移动端布局、评论与菜单快捷键行为不回归。

### 置信度评估

| 需求 | 置信度 | 说明 |
|------|--------|------|
| R2 L3/Figma 快捷键作用域 | 90% | 当前活动 frame、全局快捷键注册点与 Figma iframe 渲染点已定位；跨域边界明确，交互所有权可通过活动态与显式进入原型控制 |
